# Experimental chunked-window workflow — node breakdown

Everything on this page is on the `exp` branch and marked experimental. It is the
...
|---|---|---|
| Nodes | 2 (Windowed Conditioning + Chunked Sampler) | 4 (Start / Sample Chunk / Loop End / Assemble) |
| VAE | Sampler decodes the assembled latent itself | Yours — VAE Decode sits in the loop body |
| Per-chunk output | Nothing until the whole clip finishes | Each chunk is decoded/saved while the next one samples |
...
| `chunk_frames` | Render length per chunk in frames (default 124 — the finetune's evaluated operating point; every chunk runs at exactly this length). |
...
- With the standard ComfyUI H3 VAE, complete 17-frame encoder blocks are reused
  from the preceding window's encoding; only each window's padded tail is encoded
  fresh. The log reports how many blocks were reused. Custom VAE wrappers get the
  full-window path.
...
| Input | Meaning |
|---|---|
| `noise` / `guider` / `sampler` / `sigmas` | Your existing sampling stack (SamplerCustomAdvanced-style sockets). Stock objects support chunk reuse; opaque custom ones bypass it and rerender everything. |
| `cond_set` | From Windowed Conditioning. |
| `vae` | Used for the single final decode of the assembled video. |
| `seed` | Base seed: chunk 1 uses `seed`, chunk 2 `seed + 1`, and so on. The connected noise node's own seed is not used. |
...
| `run_name` | Checkpoint folder name: `output/viggle_chunks/<run_name>/`. 1–64 letters/digits/`_`/`-`. Use a new name for a new take; reuse it to resume. |
...
| `noise` / `guider` / `sampler` / `sigmas` | Same sampling stack as the single-pass sampler. |
...
| 3 | `status` | `STRING` | "Chunk 2 of 4: frames 102-225 (4.2-9.4s), seed 43 [saved/restored]" — put this in your Show Text |

⚠ Slot 2 is the filename prefix, slot 3 is the human-readable status. Show Text
wants **slot 3**.
...
(the latent checkpoint of that chunk was never written, so a re-queue resamples it).
...
2. Crash / cancel / OOM at chunk 3? Fix the cause, queue again with the same
   `run_name` and `resume` on: chunks 1–2 restore instantly, only the rest sample.
